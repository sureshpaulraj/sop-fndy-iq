...
function ReasoningTreeNode({ node, isLast }: { node: ReasoningNode; isLast: boolean }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        <span style={{ color: 'var(--rccb-gray-400)', fontSize: '11px', minWidth: '14px', userSelect: 'none' }}>
          {hasChildren ? (expanded ? '▼' : '▶') : isLast ? '└' : '├'}
        </span>
        <span style={{ fontSize: '12px', color: 'var(--rccb-gray-700)', lineHeight: '18px' }}>
          {node.label}
        </span>
      </div>
      {hasChildren && expanded && (
        <div style={{ marginLeft: '16px', borderLeft: '1px dashed var(--rccb-gray-300)', paddingLeft: '8px' }}>
  ...
  return (
    <div style={{ marginTop: '8px' }}>
      ...
      >
        ...
        {totalMs > 0 && <span style={{ color: 'var(--rccb-gray-500)' }}>({(totalMs / 1000).toFixed(1)}s)</span>}
      </button>
      {expanded && (
        <div
          style={{
            ...
            borderRadius: '6px',
            borderLeft: '3px solid var(--rccb-blue)',
            fontSize: '12px',
            lineHeight: '20px',
          }}
        >
          {activity.map((step, i) => (
            <div key={i} style={{ padding: '2px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ color: 'var(--rccb-gray-700)' }}>
                  <strong>{step.step}</strong>: {step.detail}
                </span>
                {step.duration_ms ? (
                  <span style={{ color: 'var(--rccb-gray-400)', fontSize: '11px', marginLeft: 'auto', flexShrink: 0 }}>
                    {step.duration_ms}ms
                  </span>
                ) : null}
              </div>
    ...
    <div style={{ marginTop: '10px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
      {prompts.map((p, i) => (
        <button
          ...
          onMouseLeave={(e) => {
            e.currentTarget.style.borderColor = 'var(--rccb-gray-300)';
            e.currentTarget.style.background = 'var(--rccb-gray-100)';
          }}
        >
          {p}
        </button>
    ...
    >
      <div
        style={{
          ...
          background: isUser ? 'var(--rccb-red)' : 'var(--rccb-white)',
          color: isUser ? 'var(--rccb-white)' : 'var(--rccb-dark)',
      ...
      >
            ...
            {message.citations.map((c) => (
              <a
                ...
                style={{
                  padding: '2px 8px',
                  borderRadius: '4px',
                  background: 'var(--rccb-blue-light, #EFF6FF)',
                  color: 'var(--rccb-blue)',
                  fontSize: '12px',
                  fontWeight: 500,
                  cursor: 'pointer',
                  border: '1px solid var(--rccb-blue)',
                  textDecoration: 'none',
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: '3px',
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = 'var(--rccb-blue-light, #EFF6FF)';
                  e.currentTarget.style.borderColor = 'var(--rccb-blue)';
                }}
        ...
        <div
          style={{
            fontSize: '11px',
            color: isUser ? 'rgba(255,255,255,0.7)' : 'var(--rccb-gray-500)',
            marginTop: '4px',
          }}
        >
